...
function pageBtn(pages) {
  ...
  for (let page = maxLeft; page < maxRight; page++) {
    pagesBtn += ` <button class="choice" data-id=${page}>${page}</button>`;
  }

  optionBtn.insertAdjacentHTML(
    "beforeend",
    `
      <button class="choice2 prev-btn">previous</button>
      ${pagesBtn}  
      <button class="choice2 next-btn">next</button>
      `
  );
}
...
function explore(e) {
  if (e.target.classList.contains("choice")) {
    ...
    collectionOfMovies.page = e.target.dataset.id;
    data = pagination(collectionOfMovies.query, collectionOfMovies.page, collectionOfMovies.rows);
    ...
    printTitles(data);
    pageBtn(parseInt(e.target.dataset.id));
  }
}
